...
The application lives in `room-booking-backend/` and uses Java 25 with Spring Boot. It is a Spring Modulith modular monolith: production code under `src/main/java/dev/ngb/backend` is organized by module first, not by technical layer. There is no top-level `controller`, `service`, `repository`, or `model` package — each of the 22 modules (`platform`, `config`, `identity`, `market`, `supply`, `booking`, and the rest; see `docs/modules/README.md`) has its own `internal/web`, `internal/service`, `internal/repository`, and `internal/model`, and `ApplicationModules.verify()` enforces that another module may only reach a module's root package, never its `internal`. Runtime configuration and Liquibase migrations live in `src/main/resources`; add database changes to `db/changelog/changes/` and register them in `db.changelog-master.yaml`. API and data-model decisions are documented in `docs/`; `docs/implementation/` documents exactly what the identity module's live endpoints do, request by request, and is the reference for any other module once it gains live code. Put tests in `src/test/java`, mirroring the production package layout. Treat `build/`, `.gradle/`, and IDE metadata as generated files.
